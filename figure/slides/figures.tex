...
\draw[red,line width=1pt] (0,-.3) -- (0,-0.5) -- (2.3,-0.5) node[midway,fill=white,font=\tiny] {\textcolor{black}{helping mechanism}} -- (2.3,-.3); 
...
\begin{tikzpicture}[
  ...
  littletext/.style={font=\sffamily\tiny,inner sep=0pt,outer sep=-2pt,fill=white},
  emptytask/.style={rectangle, minimum width=.7cm,font=\footnotesize},
  taskA/.style={fill=orange!40!yellow, draw, rectangle, minimum width=1cm,font=\footnotesize}]
...
\begin{scope}
\draw[ressty] (0,1.25) rectangle +(1.5,.5) node[midway]{res$_k$};
\smallqueue{1.3,1.5}{FIFO}
\draw[|-|] (1.5,2) -- ++(.8,0)node[yshift=0.3cm,midway,fill=white,font=\tiny]{\textcolor{red}{$|clusters|$}};
\end{scope}
...
\node[emptytask]   at(6.3,1.5) {$\cdots$};
\node[taskA] (Tx)  at(6.3,1.0) {$\tau_x$};
\node[cpusty] (P1) at(8.0,2.2) {\!$P_{i,\textcolor{red}{1}}$};
\node[emptytask]   at(8.0,1.5) {$\cdots$};
\node[cpusty] (Pc) at(8.0,0.8) {\!\!$P_{i,\textcolor{red}{c}}$};
\draw[arrow] (T1.west) to[out=180,in=0] (5.2,1.6);
\draw[arrow] (Tx.west) to[out=180,in=0] (5.2,1.4);
\draw[arrow] (T1.east) -- (7.25, 1.5) -- (P1.south west);
\draw[arrow] (Tx.east) -- (7.25, 1.5) -- (Pc.north west);
\draw[fill=gray] (7.25,1.5) circle (.1) node(G1){};
\draw[dashed, thin] ([shift={(-2.6cm,+.1cm)}]P1.north-|T1.west)node[right,xshift=.3cm,littletext]{cluster\textcolor{red}{$_i$}} rectangle ([shift={(.2,-.2)}]Pc.south east);
\draw[|-|] (3.5,1.85) -- ++(.8,0)node[midway,fill=white,font=\tiny]{\textcolor{blue}{$c$}};
\smallqueue{3.3,1.5}{FIFO}
\bigqueue{4.1,1.5}{PRIO}
\node[rotate=90,anchor=north,littletext] at(5.6,1.5){suspend};
...
\draw[arrow] (3.3,1.5)node[anchor=north east,yshift=-.2cm,font=\tiny\sffamily]{copy head} to[out=180,in=0] (2.3,1.5);
