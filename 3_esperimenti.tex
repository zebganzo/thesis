...
\subsubsection{Esperimento: confronto tra MrsP, \textit{Simple Ceiling} e \textit{no-preemption} [TODO] 
...
%la priorità è stata assegnata ad ogni task utilizzando la tecnica di \textit{rate monotonic assignment}. Liu et al.~\cite{Liu:1973:SAM:321738.321743}. afferma che, in tal caso, è possibile avvalersi di tecniche di analisi \textit{utilization-based}, secondo cui un valore di utilizzazione inferiore al 0.69 è condizione sufficiente ma non necessaria per ottenere un taskset schedulabile. Questo teorema è valido per sistemi formati da task indipendenti, quindi i job non condividono risorse durante la loro esecuzione, pertanto tale valore è tenuto in considerazione. Di conseguenza se il protocollo aggiunge elevati costi rispetto ad un'implementazione che non lo supporta ci si aspetta che sia nell'intorno di questa soglia che i due scheduler abbiano comportamenti diversi per quanto riguarda il numero di \textit{deadline miss}.\\
